...
export const getTrades = asyncHandler(async (req, res) => {
    ...
    const { realizedPnL = 0, totalTrades = 0, winningTrades = 0, losingTrades = 0, winRate = 0, avgPnl = 0, } = stats;
    const dailyTrades = (await redisService.get(`user:${userId}:dailyTrades`)) || {};
    const monthlyWinningTrades = (await redisService.get(`user:${userId}:monthlyWinningTrades`)) || {};
    return success(res, {
        trades,
        stats: {
            realizedPnL,
